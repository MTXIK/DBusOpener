# DBudOpener: D-Bus Сервис и Клиент

**DBudOpener** — это приложение на C++ с использованием Qt, позволяющее пользователям открывать файлы с помощью соответствующих приложений на основе расширений файлов. Оно состоит из двух основных компонентов:

1. **DBudOpener Service**: D-Bus сервис, который прослушивает запросы на открытие файлов и запускает соответствующее приложение.
2. **DBudOpener Client**: Клиентская программа командной строки, которая отправляет запросы на открытие файлов сервису через D-Bus.
