# FileOpener: D-Bus Сервис и Клиент

**FileOpener** — это приложение на C++ с использованием Qt, позволяющее пользователям открывать файлы с помощью соответствующих приложений на основе расширений файлов. Оно состоит из двух основных компонентов:

1. **FileOpener Service**: D-Bus сервис, который прослушивает запросы на открытие файлов и запускает соответствующее приложение.
2. **FileOpener Client**: Клиентская программа командной строки, которая отправляет запросы на открытие файлов сервису через D-Bus.
